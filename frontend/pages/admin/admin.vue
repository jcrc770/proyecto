...
export default {
  ...
  async mostracarro() {
    try {
      const response = await this.$axios.get("/carros/4uyvoOKla8Q05BoTSfVK");
  ...
  async mostrarenta() {
    try {
      const response = await this.$axios.get("/rentas/z5r7smZDHcnmwfKtL3II");
      const data = response.data;
  ...
  setActive(index) {
    ...
    if (this.menuItems[index].text === 'Car Rent') {
      this.$router.push('/admin/autos');
    }
  },
},
